...
class RoutingApolloXMLParser(ApolloXMLParser):
    ...
    def _parse_road(self, road):
        ...
        for lane_section in lanes.findall("laneSection"):
            (
                ...
                lane_succ_set,
                length,
            ) = self._parse_lane_section(lane_section, ["left", "right", "center"])
    ...
    def _parse_lane_section(
        ...
        max_length = 0
        for key in key_li:
            lane_sec_id, curr_lane_pred, curr_lane_succ, length = self._parse_lane(
                lane_section, key
            )
    ...
    def _parse_lane(
        ...
        for lane in lane_section.findall("lane"):
            length = float(lane.find("centerLine").find("geometry").get("length"))
            max_length = max(max_length, length)
