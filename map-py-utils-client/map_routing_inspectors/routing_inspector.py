...
class RoadNetwork:
    ...
    def add_road(
        self, road_id: str, length: float, parents: Set[str], children: Set[str]
    ):
...
class RoutingInspector:
    ...
    def run(self) -> Union[Dict[str, Dict], List[str]]:
        ...
        for idx, road_id_pair in enumerate(road_id_pair_list):
            ...
            curr_seg_dict["color"] = generate_unique_color(self._existing_colors)
    ...
    def res_format(self):
        ...
        if not self.has_routing:
            res_summary["first_failure_road_segment"] = self.first_fail_road_id_pair
