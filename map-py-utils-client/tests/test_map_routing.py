...
from map_routing_inspectors.routing_inspector import RoutingInspector, RoadNetwork
...
def test_astart():
    ...
    for road in parser_result.values():
        road_network.add_road(road.road_id, road.length, road.parents, road.children)

    waypoints = [road_id.rsplit("_", 1)[0] + "_0" for road_id in test_routing_list]
    results = []
