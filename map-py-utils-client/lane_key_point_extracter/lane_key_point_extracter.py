...
from utils.transfer_util import coordinate_transfer
...
class LaneKeyPointExtracter:
    ...
    def extract(self, apollo_xml: etree._Element) -> None:
        ...
        for road in roads:
            ...
            if not isinstance(lane_section_right_lanes, list):
                continue
            self.__extract_lane_key_point(lane_section_right_lanes)  # type: ignore
    ...
    def __get_head_tail_points(
        ...
        for head_point, tail_point in zip(
            ...
            head_wgs84_lon, head_wgs84_lat = float(head_point.attrib["x"]), float(  # type: ignore
                head_point.attrib["y"]  # type: ignore
            )
            head_utm_x, head_utm_y, _, _ = utm.from_latlon(
                longitude=head_wgs84_lon, latitude=head_wgs84_lat
            )
            head_gcj02_lon, head_gcj02_lat = coordinate_transfer.wgs84_to_gcj02(
            ...
            tail_wgs84_lon, tail_wgs84_lat = float(tail_point.attrib["x"]), float(  # type: ignore
                tail_point.attrib["y"]  # type: ignore
            )
            tail_utm_x, tail_utm_y, _, _ = utm.from_latlon(
                longitude=tail_wgs84_lon, latitude=tail_wgs84_lat
            )
            tail_gcj02_lon, tail_gcj02_lat = coordinate_transfer.wgs84_to_gcj02(
        ...
        return {"head_points": head_points, "tail_points": tail_points}, extrating_num
