...
class LogExtracter:
    ...
    def run(self, data: dict, auxilary_point: Tuple[float, float]):
        ...
        self._zone_number, self._zone_letter = self._get_utm_zone_info(*auxilary_point)
    ...
    def _get_utm_zone_info(self, gcj_lat: float, gcj_lon: float) -> Tuple[int, str]:
        wgs84_lon, wgs84_lat = coordinate_transfer.gcj02_to_wgs84(gcj_lon, gcj_lat)
    ...
    def _extract_single_x_y(self, data):
        if data.get("wgs84_lon") is not None:
            wgs84_lon, wgs84_lat = float(data["wgs84_lon"]), float(data["wgs84_lat"])
            utm_x, utm_y, _, _ = utm.from_latlon(wgs84_lat, wgs84_lon)
            gcj_lon, gcj_lat = coordinate_transfer.wgs84_to_gcj02(wgs84_lon, wgs84_lat)
            return {
                "utm_x": utm_x,
                "utm_y": utm_y,
                "wgs84_lat": wgs84_lat,
