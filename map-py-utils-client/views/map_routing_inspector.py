...
bp = Blueprint("map_routing_inspector", __name__, url_prefix="/map-routing-inspector")
...
ROUTING_LOCAL_BUFFER_PATH = os.path.join(current_app.instance_path, "routing_inspector")
...
def get_local_buffer_path(map_path: str, map_md5: str, buffer_type: BufferType) -> str:
    ...
    if buffer_type == BufferType.ALL_ROUTING:
        ...
        buffer_path = os.path.join(PARSER_RESULT_LOCAL_BUFFER_PATH, buffer_name)
        return buffer_path
    else:
        return ""
...
def has_local_parser_result(map_path: str, map_md5: str) -> bool:
    buffer_path = get_local_buffer_path(map_path, map_md5, BufferType.ParserResult)
    return os.path.exists(buffer_path)


def store_local_parser_result(map_path: str, map_md5: str, result: Dict) -> None:
    buffer_path = get_local_buffer_path(map_path, map_md5, BufferType.ParserResult)
...
def has_local_all_routing_result(map_path: str, map_md5: str) -> bool:
    buffer_path = get_local_buffer_path(map_path, map_md5, BufferType.ALL_ROUTING)
    return os.path.exists(buffer_path)


def store_local_all_routing_result(map_path: str, map_md5: str, result: Dict) -> None:
    buffer_path = get_local_buffer_path(map_path, map_md5, BufferType.ALL_ROUTING)
    with portalocker.Lock(buffer_path, timeout=1):
        try:
...
def map_all_routing():
    ...
    if map_path is None or map_md5 is None:
        return ResponseData(code=400, status="error", message="参数错误", data=None).json

    if has_local_all_routing_result(map_path, map_md5):
        buffer_result = get_local_all_routing_result(map_path, map_md5)
        return ResponseData(
            code=200, status="success", message="获取地图所有最大子图成功（读取缓存）", data=buffer_result
        ).json
...
def get_local_routing_inspector(map_path: str, map_md5: str) -> RoutingInspector:
    buffer_path = get_local_buffer_path(map_path, map_md5, BufferType.ROUTING)
    with open(buffer_path, "rb") as f:
        return pickle.load(f)
