...
from lane_key_point_extracter.lane_key_point_extracter import LaneKeyPointExtracter
...
def store_local_lane_key_point_extracter(
    map_path: str, map_md5: str, lane_key_point_extracter: LaneKeyPointExtracter
) -> None:
    buffer_path = get_local_buffer_path(map_path, map_md5)
...
def map_routing():
    ...
    if map_path is None or map_md5 is None or lane_uid_list is None:
        return ResponseData(code=400, status="error", message="参数不全", data=None).json
